...
2. Результаты фильтруются по выбранному пользователем набору сайтов, документов и научных статей с учётом прав владельца.
...
5. Модель получает явное правило считать содержимое источников недоверенными данными и не выполнять инструкции из них.
...
Ответ содержит `answer`, `sources`, `usedLlm` и признаки успешности. `sourceIds` ограничивает RAG выбранными источниками текущего пользователя.
