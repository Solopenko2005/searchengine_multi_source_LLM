...
var API = function(){
        ...
        startIndexing:{
            ...
            action: function(result, $this){
                if (result.result){
                    ...
                    // состояние кнопки синхронизируется по реальному состоянию индексации
                    refreshStatistics();
                } else {
                    if ($this.next('.API-error').length) {
        ...
        stopIndexing: {
            ...
            action: function(result, $this){
                if (result.result){
                    ...
                    // индексация останавливается; кнопка вернётся в исходное состояние,
                    // когда все источники фактически завершат работу
                    refreshStatistics();
                } else {
                    if ($this.next('.API-error').length) {
                        $this.next('.API-error').text(result.error);
        ...
        addSite: {
            ...
            action: function(result, $this){
                if (result.result){
                    ...
                    if ($this.next('.API-success').length) {
                        $this.next('.API-success').text(result.message || 'Источник добавлен, индексация запущена');
                    } else {
                        $this.after('<div class="API-success">' + (result.message || 'Источник добавлен, индексация запущена') + '</div>');
                    }
                } else {
                    if ($this.next('.API-success').length) {
                        $this.next('.API-success').remove();
            ...
            action: function(result, $this, data){
                if (result.result){
                    ...
                    result.data.forEach(function(page){
                        var href = page.url || (page.site + page.uri);
                        var $block = $('<div>').addClass('SearchResult-block');
                        var label = (!data.siteName ? page.siteName + ' - ' : '') + page.title;
                        $('<a>').attr('href', href)
                            .attr('target', '_blank')
                            .attr('rel', 'noopener')
                            .addClass('SearchResult-siteTitle')
                            .text(label)
                            .appendTo($block);
                        $('<div>').addClass('SearchResult-description')
                            .html(page.snippet || '')
                            .appendTo($block);
        ...
        statistics: {
            ...
            action: function(result, $this){
                if (result.result){
                    ...
                    // запоминаем раскрытые источники, чтобы автообновление их не сворачивало
                    var expandedSources = [];
                    $statistics.find('.HideBlock').not('.Statistics-example').each(function(){
                        var $b = $(this);
                        if (!$b.hasClass('HideBlock_CLOSE')) {
                            expandedSources.push($b.attr('data-source-url'));
                        }
                    });
                    ...
                    // запоминаем выбранный источник, чтобы автообновление не сбрасывало выбор
                    ...
                    result.statistics.detailed.forEach(function(site){
                        ...
                        var sourceLabel = site.name ? site.name : site.url;
                        if (site.sourceType === 'DOCUMENT') {
                        ...
                        $blockSiteExample.attr('data-source-url', site.url);
                    ...
                    // восстанавливаем раскрытые ранее блоки источников
                    $statistics.find('.HideBlock').not('.Statistics-example').each(function(){
                        var $b = $(this);
                        if (expandedSources.indexOf($b.attr('data-source-url')) !== -1) {
                            $b.removeClass('HideBlock_CLOSE');
                        }
                    });
                    // восстанавливаем ранее выбранный источник поиска (если он ещё существует)
                    if (selectedSite) {
                        $('select[name="site"]').val(selectedSite);
                    }
                    // синхронизируем состояние кнопки Начать/Остановить с реальным состоянием индексации
                    syncIndexingButton(result.statistics.total.isIndexing);
    ...
    // Запрашивает статистику и обновляет весь дашборд (в т.ч. состояние кнопки индексации)
    ...
    // Приводит кнопку «Начать/Остановить индексацию» в соответствие с реальным состоянием
    function syncIndexingButton(isIndexing){
        var $btn = $('.API-startIndexing');
        if (!$btn.length) return;
        var currentlyStopMode = $btn.data('send') === 'stopIndexing';
        if (isIndexing && !currentlyStopMode) {
            shiftCheck($btn);      // переключаем в режим «Остановить»
        } else if (!isIndexing && currentlyStopMode) {
            shiftCheck($btn);      // возвращаем в режим «Начать индексацию»
        }
    }
    return {
        init: function(){
...
var DocumentUpload = function(){
    var $documentForm = $('#documentUploadForm');

    function clearMessages($form){
        $form.find('.API-error, .API-success, .Upload-report').remove();
    }

    function showMessage($form, success, text){
        clearMessages($form);
        var cssClass = success ? 'API-success' : 'API-error';
        $form.find('.form-group_row').append('<div class="' + cssClass + '">' + text + '</div>');
    }

    // Формирует подробный отчёт по каждому загруженному файлу
    function showReport($form, result){
        ...
        if (result.files && result.files.length) {
            var html = '<div class="Upload-report"><ul class="Upload-report-list">';
        ...
        init: function(){
            ...
            $documentForm.on('submit', function(e){
                ...
                if (!files || !files.length) {
                    showMessage($form, false, 'Выберите один или несколько файлов (DOCX или PDF)');
                    return;
                }
