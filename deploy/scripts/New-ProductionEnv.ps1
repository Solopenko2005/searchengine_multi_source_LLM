...
$sourceProvider = if ($source.ContainsKey('LLM_PROVIDER')) { $source['LLM_PROVIDER'] } else { '' }
$yandexApiKey = if ($sourceProvider -match '(?i)yandex' -and $source.ContainsKey('LLM_API_KEY')) { $source['LLM_API_KEY'] } elseif ($source.ContainsKey('YANDEX_API_KEY')) { $source['YANDEX_API_KEY'] } else { '' }
$yandexFolderId = if ($source.ContainsKey('YANDEX_FOLDER_ID')) { $source['YANDEX_FOLDER_ID'] } else { '' }
...
$lines = @(
    ...
    "LLM_API_KEY=$(Format-ComposeValue $yandexApiKey)"
    "YANDEX_FOLDER_ID=$(Format-ComposeValue $yandexFolderId)"
    "LLM_MODEL=aliceai-llm"
    "LLM_FALLBACK_ENABLED=true"
    "LLM_FALLBACK_PROVIDER=local"
    "LLM_FALLBACK_BASE_URL=http://host.docker.internal:1235/v1"
    ...
    "LLM_BACKGROUND_API_KEY=$(Format-ComposeValue $yandexApiKey)"
    "YANDEX_BACKGROUND_FOLDER_ID=$(Format-ComposeValue $yandexFolderId)"
    "LLM_BACKGROUND_MODEL=yandexgpt-5.1"
    "LLM_BACKGROUND_PORT=1236"
    "LLM_BACKGROUND_CPU_THREADS=2"
    "LLM_BACKGROUND_CONTEXT_SIZE=4096"
    "LLM_BACKGROUND_NICE=10"
    ...
    "OPENAI_QUEUE_TIMEOUT_MILLIS=3000"
    "OPENAI_BACKGROUND_TIMEOUT_SECONDS=45"
    "OPENAI_BACKGROUND_MAX_OUTPUT_TOKENS=192"
