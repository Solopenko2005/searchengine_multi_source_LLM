...
пользователей. Основные ответы формирует Yandex AI Studio, поэтому CPU сервера
занимается поиском и индексированием. Qwen3 4B остаётся локальным резервом на
случай временной ошибки облачного API.
...
В [интерфейсе AI Studio](https://aistudio.yandex.cloud/) создайте сервисный
аккаунт и API-ключ. Аккаунту необходима роль `ai.languageModels.user`. Запишите в
`.env.production` секретное значение ключа и идентификатор каталога:
...
Не сохраняйте ключ в Git и не передавайте его через браузер приложения. После
запуска `/api/assistant/status` должен показать провайдера `Yandex AI Studio` и
модель `aliceai-llm`.

## 4. Локальный резерв и embeddings без оплаты токенов
...
1235 с восемью CPU-потоками, а компактная Qwen3-0.6B на 1236 выполняет только
классификацию тематик с двумя потоками и пониженным приоритетом ОС. Такое разделение
...
Обе службы запускаются с отключённым скрытым reasoning: RAG уже передаёт отобранный
контекст, поэтому модель сразу печатает проверяемый ответ или структурированный JSON.
