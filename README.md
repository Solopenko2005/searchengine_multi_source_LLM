...
## Локальный запуск вместе с сервисом авторизации

Расположите каталоги `searchengine_multi_source_LLM` и `social-network-authorization`
рядом в `IdeaProjects`. Заполните `.env`, затем выполните из корня поискового проекта:

```powershell
powershell -ExecutionPolicy Bypass -File .\run-local.ps1
```

Скрипт запускает сервис авторизации на порту `5555`, ждёт его готовности и запускает
поисковую систему на порту `8080`. Регистрация доступна по адресу
`http://localhost:8080/register`. В локальной конфигурации зарегистрированному
пользователю назначается роль `ADMIN`.

Для публичного развёртывания следует поменять роль регистрации по умолчанию на
`USER` и добавить отдельное подтверждение администратора.
